...
//FALTA COMPLETAR TOTALMENTE, FALTA COISAAAAS
    public function listarPostagem($idGrupo) {
        $db = $this->dbConn();
        $stmt = $db->prepare("SELECT AG.CD_ALUNO_GRUPO,A.CD_MATRICULA, A.NM_ALUNO, A.img_aluno, P.DS_POSTAGEM,P.ARQUIVO_POSTAGEM, P.TIPO_POSTAGEM_cd_tipo_postagem, P.CD_POSTAGEM  FROM ALUNO_GRUPO AG, ALUNO A, POSTAGEM P WHERE  AG.CD_GRUPO = ? AND P.CD_ALUNO_GRUPO = AG.CD_ALUNO_GRUPO AND AG.CD_MATRICULA = A.CD_MATRICULA AND P.IC_POSTAGEM = 1 order by p.dt_postagem desc");
